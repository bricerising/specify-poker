...
export function createUnaryCallResultProxy<TClient extends object>(
  client: TClient,
  ...
  return createUnaryClientProxy<UnaryCallResultProxy<TClient>, Promise<UnaryCallResult<unknown>>>({
    ...
    onNonFunctionProperty: (prop) =>
      Promise.resolve(
        err(new Error(`unary_call_result_proxy.non_function_property:${String(prop)}`)),
      ),
  });
}
