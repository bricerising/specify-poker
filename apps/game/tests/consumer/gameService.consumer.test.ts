...
vi.mock("../../src/api/grpc/clients", () => ({
  balanceClient: {
    ...
    CommitReservation: (request: unknown, callback: (err: Error | null, response?: unknown) => void) => {
    ...
    ReleaseReservation: (request: unknown, callback: (err: Error | null, response?: unknown) => void) => {
      grpcState.releaseCalls.push(request);
      callback(null, { ok: true });
    },
    ProcessCashOut: (request: unknown, callback: (err: Error | null, response?: unknown) => void) => {
...
import { TableConfig } from "../../src/domain/types";
...
const resetRedisState = () => {
  const moduleWithReset = redisClientModule as typeof redisClientModule & { __resetRedis?: () => void };
  moduleWithReset.__resetRedis?.();
};

const clearTurnTimers = () => {
  const service = tableService as typeof tableService & { turnTimers?: Map<string, NodeJS.Timeout> };
...
describe("gRPC handler consumer flows", () => {
  ...
  it("returns hole cards only to the owning player", async () => {
    ...
    const playerView = await callUnary(handlers.GetTableState, { table_id: table.tableId, user_id: "player-1" });
    expect(playerView.err).toBeNull();
    expect(playerView.response?.hole_cards.length).toBe(2);

    const spectatorView = await callUnary(handlers.GetTableState, { table_id: table.tableId, user_id: "spectator-1" });
