...
export function createHandlers() {
  return {
    CreateTable: createGameUnaryHandler(
      'CreateTable',
      async ({ name, owner_id, config }: CreateTableRequest) => {
    ...
    ListTables: createGameUnaryHandler<ListTablesRequest, ListTablesResponse>(
      'ListTables',
      async (_request) => {
    ...
    JoinSeat: createGameUnaryHandler(
      'JoinSeat',
      ({ table_id, user_id, seat_id, buy_in_amount }: JoinSeatRequest) => {
    ...
    JoinSpectator: createGameUnaryHandler(
      'JoinSpectator',
      ({ table_id, user_id }: JoinSpectatorRequest) =>
        tableService.joinSpectator(table_id, user_id),
    ),

    LeaveSpectator: createGameUnaryHandler(
      'LeaveSpectator',
      ({ table_id, user_id }: LeaveSpectatorRequest) =>
    ...
    KickPlayer: createGameUnaryHandler(
      'KickPlayer',
      async ({ table_id, owner_id, target_user_id }: KickPlayerRequest) => {
    ...
    MutePlayer: createGameUnaryHandler(
      'MutePlayer',
      async ({ table_id, owner_id, target_user_id }: MutePlayerRequest) => {
    ...
    UnmutePlayer: createGameUnaryHandler(
      ...
      async ({ table_id, owner_id, target_user_id }: UnmutePlayerRequest) => {
        const result = await moderationService.mutePlayer(
          table_id,
          owner_id,
          target_user_id,
          false,
