...
export class HandStore {
  ...
  async getHandHistory(tableId: string, limit = 20, offset = 0): Promise<{ hands: HandRecord[]; total: number }> {
    const countRes = await pool.query("SELECT COUNT(*) FROM hand_records WHERE table_id = $1", [tableId]);
  ...
  async getHandsForUser(userId: string, limit = 20, offset = 0): Promise<{ hands: HandRecord[]; total: number }> {
    const countRes = await pool.query("SELECT COUNT(*) FROM hand_records WHERE participants @> $1", [
