...
export function createHandlers() {
  return {
    ...
    getHandRecord: unary<GetHandRecordRequest, unknown>(async (request) => {
      ...
      const record = await handRecordService.getHandRecord(handId, optionalNonEmptyString(request.requesterId));
      if (!record) {
    ...
    getHandHistory: unary<GetHandHistoryRequest, unknown>(async (request) => {
      ...
      const result = await handRecordService.getHandHistory(tableId, limit, offset, optionalNonEmptyString(request.requesterId));
      return {
    ...
    getHandReplay: unary<GetHandReplayRequest, unknown>(async (request) => {
      const handId = requireNonEmptyString(request.handId, "handId");
      const events = await replayService.getHandEvents(handId, optionalNonEmptyString(request.requesterId));
            ...
            if (streams) {
              for (const stream of streams) {
                for (const message of stream.messages) {
                  ...
                  if (!parsed) {
                    logger.error({ streamId, messageId: message.id }, "Invalid JSON in Redis stream message");
                    continue;
                  }
...
function timestampToDate(timestamp?: GrpcTimestamp): Date | undefined {
  ...
  const nanos = typeof timestamp.nanos === "number" && Number.isFinite(timestamp.nanos) ? timestamp.nanos : 0;
...
function safeJsonParse(raw: string): StreamEventJson | null {
  try {
    ...
    if (typeof eventId !== "string" || typeof type !== "string" || typeof tableId !== "string" || !isRecord(payload)) {
      return null;
    }
