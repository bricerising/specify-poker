...
import { asGrpcServiceError, createGrpcServiceError, isGrpcServiceErrorLike } from "@specify-poker/shared";
...
export function toServiceError(error: unknown): grpc.ServiceError {
  ...
  if (error instanceof InvalidArgumentError) {
    return createGrpcServiceError(grpc.status.INVALID_ARGUMENT, message, error) as grpc.ServiceError;
  }
  if (error instanceof NotFoundError) {
    return createGrpcServiceError(grpc.status.NOT_FOUND, message, error) as grpc.ServiceError;
  }
  if (error instanceof PermissionDeniedError) {
    return createGrpcServiceError(grpc.status.PERMISSION_DENIED, message, error) as grpc.ServiceError;
  }
