...
describe("gRPC handlers consumer flows", () => {
  ...
  it("returns nicknames for batch nickname lookup", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("GetNicknames", "ok", expect.any(Number));
  });
  ...
  it("normalizes empty avatar URLs on profile updates", async () => {
    ...
    } as unknown as ServerUnaryCall<{ userId: string; nickname?: string; avatarUrl?: string }, unknown>;
  ...
  it("returns error for invalid statistic type", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("IncrementStatistic", "error", expect.any(Number));
  ...
  it("increments statistics with a valid type", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("IncrementStatistic", "ok", expect.any(Number));
  });

  it("supports friend removal via gRPC", async () => {
    const call = { request: { userId: "user-1", friendId: "user-2" } } as unknown as ServerUnaryCall<
  ...
  it("records errors when profile lookup fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("GetProfile", "error", expect.any(Number));
  });
  ...
  it("records errors when friend mutation fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("AddFriend", "error", expect.any(Number));
  });
  ...
  it("records errors when delete fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("DeleteProfile", "error", expect.any(Number));
  });
  ...
  it("records errors when statistics lookup fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("GetStatistics", "error", expect.any(Number));
  });
  ...
  it("records errors when batch lookup fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("GetProfiles", "error", expect.any(Number));
  });
  ...
  it("records errors when friends lookup fails", async () => {
    ...
    expect(metrics.recordGrpcRequest).toHaveBeenCalledWith("GetFriends", "error", expect.any(Number));
  });
});
