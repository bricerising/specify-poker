...
describe("Player service consumer architecture", () => {
  ...
  it("rejects invalid avatar URLs on profile updates", async () => {
    ...
    try {
      ...
      await expect(profileService.updateProfile("user-1", { avatarUrl: "not-a-url" })).rejects.toThrow(
  ...
  it("returns friend profiles with nicknames and avatars", async () => {
    ...
    const summariesSpy = vi.spyOn(profileService, "getProfileSummaries").mockResolvedValue(summaries);
