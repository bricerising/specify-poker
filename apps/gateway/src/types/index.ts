...
export interface GameServiceClient {
  GetTableState(
    ...
    callback: UnaryCallback<{ state: TableState; hole_cards?: Card[] }>,
  ): void;
  ...
  IsMuted(
    request: { table_id: string; user_id: string },
    callback: UnaryCallback<{ is_muted: boolean }>,
  ): void;
}

export interface PlayerServiceClient {
  GetProfile(
    request: { user_id: string; referrer_id?: string; username?: string },
  ...
  GetNicknames(
    ...
    callback: UnaryCallback<{ nicknames: Array<Record<string, unknown>> }>,
  ): void;
}
...
export interface EventServiceClient {
  ...
  GetHandRecord(
    request: { hand_id: string; requester_id?: string },
    callback: UnaryCallback<Record<string, unknown>>,
  ): void;
  GetHandReplay(
    request: { hand_id: string },
    callback: UnaryCallback<{ hand_id: string; events: unknown[] }>,
  ): void;
  GetHandHistory(
