...
describe('Chat WS handler', () => {
  ...
  it('rejects empty chat messages', async () => {
    ...
    socket.emit(
      'message',
      Buffer.from(JSON.stringify({ type: 'ChatSend', tableId: 't1', message: '' })),
    );
    await flushPromises();
  ...
  it('rate limits chat messages at the gateway', async () => {
    ...
    socket.emit(
      'message',
      Buffer.from(JSON.stringify({ type: 'ChatSend', tableId: 't1', message: 'hello' })),
    );
    await flushPromises();
  ...
  it('rejects muted users', async () => {
    vi.mocked(gameClient.GetTableState).mockImplementation((_req, callback) => {
      callback(null, {
        state: { seats: [{ user_id: 'user-1', status: 'active' }], spectators: [] },
      });
    });
  ...
  it('broadcasts chat messages for spectators', async () => {
    vi.mocked(gameClient.GetTableState).mockImplementation((_req, callback) => {
      callback(null, {
        state: { seats: [], spectators: [{ user_id: 'user-1', status: 'active' }] },
      });
    });
    vi.mocked(gameClient.IsMuted).mockImplementation((_req, callback) => {
