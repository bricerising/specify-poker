...
(0, vitest_1.describe)("friends endpoints", () => {
    (0, vitest_1.it)("stores and returns friends list", async () => {
        ...
        (0, vitest_1.expect)(updateResponse.body).toEqual({ friends: ["friend-a", "friend-b"] });
        const listResponse = await (0, supertest_1.default)(app)
            .get("/api/friends")
            .set("Authorization", `Bearer ${token}`);
        (0, vitest_1.expect)(listResponse.status).toBe(200);
        (0, vitest_1.expect)(listResponse.body).toEqual({ friends: ["friend-a", "friend-b"] });
        const profileResponse = await (0, supertest_1.default)(app)
            .get("/api/me")
            .set("Authorization", `Bearer ${token}`);
        (0, vitest_1.expect)(profileResponse.body.friends).toEqual(["friend-a", "friend-b"]);
    });
});
