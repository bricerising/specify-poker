...
async function waitForClose(socket: WebSocket, timeoutMs = 8000): Promise<{ code: number; reason: string }> {
...
test.describe("Gateway Auth Boundary (HTTP + WebSocket)", () => {
  ...
  test("enforces JWT auth for WebSocket connections", async () => {
    ...
    const noTokenSocket = new WebSocket(gatewayWsUrl(), { headers: { "X-Forwarded-For": forwardedFor } });
    const noTokenClosed = await waitForClose(noTokenSocket);
    expect(noTokenClosed.code).toBe(1008);

    const invalidTokenSocket = new WebSocket(gatewayWsUrl("not-a-jwt"), { headers: { "X-Forwarded-For": forwardedFor } });
    ...
    const okSocket = new WebSocket(gatewayWsUrl(token), { headers: { "X-Forwarded-For": forwardedFor } });
    const welcome = await new Promise<Record<string, unknown>>((resolve, reject) => {
      ...
      okSocket.on("message", (data) => {
        try {
          ...
          if (parsed && typeof parsed === "object" && (parsed as { type?: unknown }).type === "Welcome") {
            clearTimeout(timer);
