...
describe('reservationService', () => {
  ...
  describe('reserveForBuyIn', () => {
    it('reserves funds for buy-in', async () => {
      ...
      const result = await reserveForBuyIn('user-1', 'table-1', 500, 'reserve-1');
    ...
    it('rejects reservation with insufficient balance', async () => {
      ...
      const result = await reserveForBuyIn('user-2', 'table-1', 500, 'reserve-2');
    ...
    it('rejects reservation for non-existent account', async () => {
      const result = await reserveForBuyIn('nonexistent', 'table-1', 500, 'reserve-3');
