...
describe('End-to-End Balance Flows', () => {
  ...
  describe('Two-Phase Buy-In Flow', () => {
    ...
    it('handles buy-in cancellation: reserve → release', async () => {
      ...
      const reserveResult = await reserveForBuyIn('player-2', 'table-xyz', 2000, 'reserve-key-2');

      expect(reserveResult.ok).toBe(true);
      expect(reserveResult.availableBalance).toBe(3000);

      // Phase 2: Release (table full, seat taken, etc.)
      const releaseResult = await releaseReservation(reserveResult.reservationId!, 'seat_taken');
  ...
  describe('Cash-Out Flow', () => {
    ...
    it('prevents cash-out with active reservation', async () => {
      ...
      const reserveResult = await reserveForBuyIn('held-player', 'table-new', 1000, 'reserve-new');
      expect(reserveResult.ok).toBe(true);
      ...
      const validWithdraw = await processWithdrawal('held-player', 1000, 'valid-withdraw');
